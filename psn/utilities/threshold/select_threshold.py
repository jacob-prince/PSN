...
def select_threshold(cSb, cNb, ntrials, opt, device='cpu', tie_eps=1e-3):
    ...
    basis is chosen by analytic recovery against the GSN covariances.
    ...
    <opt> - dict. Reads <basis> and <criterion> (see psn for full descriptions):
        <basis>
          'compare'    -> build both the signal and difference bases, each at its
                          ...
                          higher analytic recovery. A margin within <tie_eps>
                          keeps 'signal' (the more near-unbiased basis). The
                          empirical split-half reliability of each candidate is
                          recorded under record['diagnostics'] but does not affect
                          the choice.
    ...
    <tie_eps> - scalar. Recovery margin within which 'compare' keeps 'signal'.
    ...
    # --- compare: signal vs difference (configured criterion), analytic pick,
    #     tie-break to signal -------------------------------------------------
    if basis == 'compare':
        cands = {b: _select_lowrank(cSb, cNb, t, b, criterion, opt, device=device)
                 for b in ('signal', 'difference')}
        rec_sig = cands['signal']['recovery']
        rec_dif = cands['difference']['recovery']
        # prefer 'signal' (more near-unbiased) unless 'difference' wins by > eps
        winner = 'difference' if (rec_dif - rec_sig) > tie_eps else 'signal'
        rec = dict(cands[winner])
        rec['diagnostics'] = {
            'mode': 'compare', 'picked_by': 'analytic_recovery', 'tie_eps': tie_eps,
            'candidates': {b: {'best_threshold': cands[b]['best_threshold'],
                               'recovery': cands[b]['recovery'],
                               'sv_frac': cands[b]['sv_frac']} for b in cands},
            # both candidate bases, so the figure reuses them instead of re-eigh-ing;
            # split_half_* are added by the figure layer (out-of-sample validation only)
